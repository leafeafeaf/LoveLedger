...
export default function CoverSelectionScreen({ navigation, route }) {
  ...
  const coverStyles = [
    ...
    { id: 'vintage', label: 'Vintage', icon: 'camera-vintage' },
  ...
  const generateCover = () => {
    ...
    const seed = coverStyles.findIndex(style => style.id === selectedStyle) + 100;
    ...
    const imageUrl = `https://api.a0.dev/assets/image?text=${encodeURIComponent(prompt)}&aspect=3:4&seed=${seed}`;
          ...
          {coverStyles.map((style) => (
            ...
            >
              <MaterialCommunityIcons 
                name={style.icon} 
                size={28} 
                color={selectedStyle === style.id ? theme.colors.white : theme.colors.primary} 
              />
              <Text style={[
                styles.styleLabel,
