...
export default function LinkErrorScreen({ navigation, route }) {
  ...
  const getErrorDetails = () => {
    switch(errorType) {
      case 'expired':
        return {
          title: '만료된 링크',
          message: '이 초대 링크는 만료되었습니다. 배우자에게 새로운 링크를 요청해 주세요.',
          icon: 'clock-alert',
        };
      case 'invalid':
        return {
          title: '유효하지 않은 링크',
          message: '이 초대 링크는 유효하지 않습니다. 올바른 링크인지 확인해 주세요.',
          icon: 'link-variant-off',
        };
      case 'already_linked':
        return {
          title: '이미 연동됨',
          message: '귀하는 이미 다른 사용자와 연동되어 있습니다. 관리자에게 문의하여 기존 연동을 해제한 후 다시 시도해 주세요.',
          icon: 'account-multiple',
        };
      default:
        return {
          title: '오류 발생',
          message: '연동 과정에서 오류가 발생했습니다. 다시 시도하거나 관리자에게 문의해 주세요.',
          icon: 'alert-circle',
        };
    }
  };
    ...
    <View style={styles.container}>
      ...
      >
        <View style={styles.errorIconContainer}>
          <MaterialCommunityIcons name={errorDetails.icon} size={80} color={theme.colors.error} />
        </View>
