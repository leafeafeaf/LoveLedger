...
export default function SeriesSelectionScreen({ navigation, route }) {
  ...
  const mockSeries = [
    { id: 1, title: 'Our Love Journey', episodes: 3, lastUpdated: '2024-03-10' },
    { id: 2, title: 'Weekend Adventures', episodes: 5, lastUpdated: '2024-03-05' },
    { id: 3, title: 'Romantic Escapes', episodes: 2, lastUpdated: '2024-02-28' },
  ];

  const renderSeriesItem = ({ item }) => (
    <Pressable 
      style={[styles.seriesCard, selectedSeries === item.id && styles.selectedSeriesCard]}
  ...
  const handleNext = () => {
    let seriesData;
    if (mode === 'new') {
      // Create new series with the entered name
      seriesData = { name: newSeriesName || 'Our Love Story' };
    } else {
      // Use the selected existing series
      const series = mockSeries.find(s => s.id === selectedSeries);
      seriesData = { id: selectedSeries, name: series?.title };
    }
      ...
      <View style={styles.optionsContainer}>
        ...
        >
          <MaterialCommunityIcons 
            name="bookshelf" 
            size={24} 
            color={mode === 'existing' ? theme.colors.white : theme.colors.primary} 
          />
          <Text style={[
            styles.modeButtonText,
        ...
        {mode === 'new' ? (
          <View style={styles.newSeriesContainer}>
            ...
            <Text style={styles.description}>
              Create a new series to organize your stories. You can add more episodes to this series later.
            </Text>
          </View>
        ) : (
