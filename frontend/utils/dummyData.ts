...
const generateTransactionsForDateRange = (startDate: Date, endDate: Date, userId?: string) => {
  const transactions: Transaction[] = [];
  const categories = ['식비', '카페', '마트/편의점', '문화/여가', '쇼핑', '교통', '병원', '여행'];
  const stores = {
  ...
  while (currentDate <= endDate) {
    ...
    for (let i = 0; i < numTransactions; i++) {
      ...
      transactions.push({
        ...
        time: `${currentDate.toISOString().split('T')[0]}T${String(Math.floor(Math.random() * 24)).padStart(2, '0')}:${String(Math.floor(Math.random() * 60)).padStart(2, '0')}:00`,
        remittance: Math.random() > 0.2, // 80% chance of being an expense
        targetname: store,
        category: category,
        afteramount: 1000000, // Placeholder
        amount: amount,
        userId: userId
      });
    }
...
export const myTransactions = generateTransactionsForDateRange(startDate, endDate, 'user1');
export const partnerTransactions = generateTransactionsForDateRange(startDate, endDate, 'user2');
...
const generateDailyFinance = (transactions: Transaction[], userId?: string) => {
  ...
  return dailyFinance.sort((a, b) => new Date(b.day).getTime() - new Date(a.day).getTime());
};

export const myDailyFinance = generateDailyFinance(myTransactions, 'user1');
export const partnerDailyFinance = generateDailyFinance(partnerTransactions, 'user2');
